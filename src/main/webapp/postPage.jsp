<%-- 
    Document   : postPage
    Created on : 28 Dec, 2012, 12:43:56 AM
    Author     : rishabh
--%>
...
<%
    ...
    header.addScript("scripts/postPageJs.js");
...
%>
            <a href="coursePage.jsp?courseId=<%=courseId%>&offeringId=<%=elem.offeringId%>"
               id="coursePage.jsp?courseId=<%=courseId%>&offeringId=<%=elem.offeringId%>">
                <li><%=elem.year +  " " + elem.semester + " " + elem.offeringId%></li>
            </a>  
<%
        }
    }
...
%>  
            <a href="feedback.jsp?offeringId=<%=offeringId%>&instructorId=<%=newEntry.instructor_id%>"
               id="instructorId<%=newEntry.instructor_id%>">
                <li><%=newEntry.instructor_id%>&nbsp;<%=newEntry.instructor_name%></li>
            </a>
<%
       }
    }
...
%>
    ...
    <div id="offeringPosts">
        <ul class="posts">
<%
    if (offeringId != -1 && postId != -1){
        ArrayList<postInfo> postList = coursePage.getSinglePost(postId);
        if (postList != null){
